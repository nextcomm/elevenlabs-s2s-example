...
const DEFAULT_INBOUND_AGENTS = {
  '554135146137': 'agent_5201kw06cme9fat8d3t42drcfd5z' // Pitch Comercial | Nextcomm
};
const DEFAULT_INBOUND_GREETINGS = {
  '554135146137': 'Oi, tudo bem? Obrigado por ligar. Como posso te ajudar?'
};
...
const service = ({logger, makeService, app}) => {
  ...
  svc.on('session:new', async (session, path) => {
    ...
    // Ordem: agente da chamada (saída) → agente do DID chamado (entrada) → fixo.
    const isInbound = session.direction === 'inbound';
    const didAgent = isInbound ? inboundAgentFor(session.to) : null;
    const agent_id = session.customerData?.agent_id || didAgent || process.env.ELEVENLABS_AGENT_ID;
    const api_key = process.env.ELEVENLABS_API_KEY;

    if (isInbound) {
      session.locals.logger.info(
        {to: session.to, agent_id, porDid: Boolean(didAgent)},
    ...
    if (hasName) {
      ...
      const saudacao = inboundGreetingFor(session.to);
      if (saudacao) {
        clientData.conversation_config_override = { agent: { first_message: saudacao } };
      }
    }
