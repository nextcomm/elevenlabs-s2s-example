...
// O ElevenLabs/Jambonz não executa a transferência neste canal (WS custom), então
// o bridge trata a tool e REDIRECIONA a chamada para discar o humano via verbo
// `dial`. A BINA (callerId) TEM que ser a identidade REGISTRADA do tronco (ex.:
// "4133305800" SEM o 55) — senão o softswitch recusa com "403 Forbidden auth ID".
// Tudo é env-driven, com os defaults do Sebrae:
// O redirect do Jambonz SÓ aceita call_hook HTTP (ws-requestor: só faz handover se
// url.startsWith('http')). Depois do handover, hooks residuais do LLM (ex.: /final)
// são buscados como HTTP na nova base — por isso existe a rota coringa que devolve [].
...
const service = ({logger, makeService, app}) => {
  ...
  // Rota HTTP de transferência: o onToolCall injeta um `redirect` (call_hook HTTP)
  // da chamada para cá; devolvemos o verbo `dial` que conecta o cliente ao atendente
  // humano, com a bina = identidade registrada do tronco (senão o softswitch dá 403).
  app.post('/transfer-dial', (req, res) => {
    logger.info({ number: TRANSFER_NUMBER, callerId: TRANSFER_CALLER_ID, trunk: TRANSFER_TRUNK }, 'transfer-dial: discando humano');
    res.json([
      {
        verb: 'dial',
        ...
        answerOnBridge: true,
        timeout: 30,
        target: [{ type: 'phone', number: TRANSFER_NUMBER, trunk: TRANSFER_TRUNK }]
      }
    ]);
  });

  // Coringa: depois do handover para /transfer-dial, o Jambonz pode buscar hooks
  // residuais do LLM (ex.: /final, /event) como HTTP na nova base — o que dava 404 e
  // derrubava a chamada antes do dial. Aqui devolvemos [] (sem mais verbos) para
  // qualquer POST não mapeado, deixando o `dial` seguir. Fica por ÚLTIMO.
...
const onToolCall = async(session, evt) => {
  ...
  // Transferência para humano. O ElevenLabs não executa transferência neste canal,
  // então NÓS a fazemos: REDIRECIONAMOS a chamada (call_hook HTTP) para /transfer-dial,
  // que devolve o verbo `dial` conectando o cliente ao atendente humano.
  // A client tool tem expects_response=false → NÃO enviamos sendToolOutput (isso ainda
  // causava um TypeError no processToolOutput do Jambonz). Só redirecionamos.
  if (name === 'transfer_to_number' || name === 'transferir_humano') {
    const call_hook = `${PUBLIC_BASE_URL}/transfer-dial`;
    logger.info({ call_hook, call_sid: session.call_sid }, 'transferindo: redirecionando chamada para discar humano');
    try {
