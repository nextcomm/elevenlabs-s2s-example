...
const service = ({logger, makeService, app}) => {
  ...
  // Hook de transferência: quando a Bel pede transferência, o onToolCall injeta um
  // `redirect` da chamada para cá; devolvemos o verbo `dial` que conecta o cliente
  // ao atendente humano. Parametros vêm na query (com defaults do Sebrae).
  app.post('/transfer-dial', (req, res) => {
    const number = req.query.number || TRANSFER_NUMBER;
    const callerId = req.query.callerId || TRANSFER_CALLER_ID;
    const trunk = req.query.trunk || TRANSFER_TRUNK;
    logger.info({ number, callerId, trunk, call_sid: req.body && req.body.call_sid }, 'transfer-dial: discando humano');
    res.json([
      {
        verb: 'dial',
        callerId,
        answerOnBridge: true,
        timeout: 30,
        target: [{ type: 'phone', number, ...(trunk ? { trunk } : {}) }]
      }
    ]);
  });
...
const onToolCall = async(session, evt) => {
  ...
  // então NÓS a fazemos: confirmamos a tool e REDIRECIONAMOS a chamada para o hook
  // /transfer-dial, que devolve o verbo `dial` (com a bina/identidade correta).
  // Aceita tanto a tool de sistema `transfer_to_number` quanto uma client tool
  // `transferir_humano`. O número/bina pode vir nos args; senão usa os defaults.
  if (name === 'transfer_to_number' || name === 'transferir_humano') {
    ...
    const number = (args && (args.phone_number || args.number)) || TRANSFER_NUMBER;
    const callerId = (args && args.caller_id) || TRANSFER_CALLER_ID;
    const q = new URLSearchParams({ number, callerId, trunk: TRANSFER_TRUNK }).toString();
    const call_hook = `${PUBLIC_BASE_URL}/transfer-dial?${q}`;
