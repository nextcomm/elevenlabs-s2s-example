...
async function quotaReport(number, seconds) {
  if (!QUOTA_BASE || !QUOTA_SECRET || !number || !(seconds > 0)) return;
  try {
...
const service = ({logger, makeService, app}) => {
  ...
  svc.on('session:new', async (session, path) => {
    ...
    session.locals.logger.info({ dynamicVars }, 'variáveis dinâmicas para esta chamada');
    ...
    if (session.direction === 'inbound') {
      ...
      if (q && q.ownerFound) {
        session.locals.quotaNumber = session.to; // medir no fim
        if (typeof q.remainingSeconds === 'number' && q.remainingSeconds > 0) {
...
const onClose = (session, code, reason) => {
  ...
  if (session.locals.quotaNumber && session.locals.startedAt) {
    const seconds = Math.round((Date.now() - session.locals.startedAt) / 1000);
    quotaReport(session.locals.quotaNumber, seconds);
  }
};
