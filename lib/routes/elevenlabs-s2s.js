...
// O redirect TEM que ser WebSocket (wss), não HTTP: o app é WS e um call_hook HTTP
// faz o Jambonz trocar a base e buscar os hooks (ex.: /final) como HTTP → 404.
const PUBLIC_WS_BASE = process.env.PUBLIC_WS_BASE || 'wss://elevenlabs-s2s-example-production.up.railway.app';
...
const service = ({logger, makeService, app}) => {
  ...
  // Serviço WS de transferência: o onToolCall injeta um `redirect` da chamada para
  // ESTA rota (wss); aqui a gente discа o atendente humano com o verbo `dial`,
  // usando a bina = identidade registrada do tronco (senão o softswitch dá 403).
  // É WS (não HTTP) de propósito — ver comentário do PUBLIC_WS_BASE.
  const xfer = makeService({path: '/transfer'});
  xfer.on('session:new', (session) => {
    session.locals = { ...session.locals, logger: logger.child({ call_sid: session.call_sid, xfer: true }) };
    session.locals.logger.info(
      { number: TRANSFER_NUMBER, callerId: TRANSFER_CALLER_ID, trunk: TRANSFER_TRUNK },
      'transfer: discando atendente humano'
    );
    session
      .dial({
...
const onToolCall = async(session, evt) => {
  ...
  // então NÓS a fazemos: confirmamos a tool e REDIRECIONAMOS a chamada para a rota
  // WS `/transfer`, que discа o atendente humano (verbo `dial` com a bina correta).
  // O redirect é para wss (não HTTP) — senão o Jambonz busca os hooks como HTTP e dá 404.
  if (name === 'transfer_to_number' || name === 'transferir_humano') {
    if (tool_call_id) {
      session.sendToolOutput(tool_call_id, {
        type: 'client_tool_result',
        tool_call_id,
        result: 'transferindo para um atendente humano agora',
        is_error: false
      });
    }
    const call_hook = `${PUBLIC_WS_BASE}/transfer`;
