...
// Destino/tronco/BINA da transferência: o normal é vir POR NÚMERO do app
// (/api/quota/check → transferNumber/transferTrunk/transferCallerId), resolvido em
// session:new e guardado em session.locals.transfer. As envs abaixo são só o
// fallback legado (Bel) para quando o app estiver fora. Assim, cadastrar a
// transferência de um cliente novo é só preencher o campo na tela de Números —
// nada de mexer aqui.
...
// o 404 do verbo `dial` a partir de chamada de entrada. `cfg` traz o destino
// resolvido por número (com fallback nas envs).
async function createSpecialistLeg(conf, cfg, logger) {
  // O app guarda o destino em dígitos ("551135121849"); a chamada validada usa
  // E.164 com "+". Normaliza: se vier só dígitos, prefixa "+" (o operador coloca
  // DDI+DDD, conforme o placeholder da tela). O fallback de env já vem com "+".
  const rawNumber = (cfg && cfg.number) || TRANSFER_NUMBER;
  const number = String(rawNumber).startsWith('+')
    ? rawNumber
    : '+' + String(rawNumber).replace(/\D/g, '');
  const trunk = (cfg && cfg.trunk) || TRANSFER_TRUNK;
  const callerId = (cfg && cfg.callerId) || TRANSFER_CALLER_ID;
  const url = `${JAMBONZ_ADMIN_URL}/Accounts/${JAMBONZ_VM_ACCOUNT}/Calls`;
  const body = {
    from: callerId,
    to: { type: 'phone', number, trunk },
  ...
  logger.info({ conf, status: r.status, to: number, trunk, callerId }, 'createSpecialistLeg: perna do especialista criada');
}
...
const service = ({logger, makeService, app}) => {
  ...
  svc.on('session:new', async (session, path) => {
    ...
    if (isInbound) {
      ...
      // Transferência para humano DESTE número (configurada na tela de Números):
      // destino + tronco + BINA. Se o app não trouxe a BINA, usa o próprio número
      // chamado (identidade registrada do tronco). O onToolCall lê isto; sem config
      // do app, cai no fallback de env (Bel legado).
      session.locals.transfer = {
        number: (inbound && inbound.transferNumber) || null,
        trunk: (inbound && inbound.transferTrunk) || null,
        callerId: (inbound && inbound.transferCallerId) || normalizeDid(session.to) || null,
      };
    }
...
const onToolCall = async(session, evt) => {
  ...
  if (name === 'transfer_to_number' || name === 'transferir_humano') {
    ...
    // destino resolvido por número (app) com fallback nas envs
    const cfg = session.locals.transfer || {};
    // perna do especialista (fire-and-forget — não trava a conversa se demorar)
    createSpecialistLeg(conf, cfg, logger).catch((err) =>
    ...
    logger.info({ conf, call_hook, call_sid: session.call_sid, destino: cfg.number || TRANSFER_NUMBER }, 'transferindo: cliente entrando na conferencia');
    try {
