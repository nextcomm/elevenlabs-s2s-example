...
module.exports = ({ logger, app }) => {
  app.all('/call-status', async (req, res) => {
    ...
    try {
      ...
      logger.info({ body }, 'full payload received');

      // Jambonz pode enviar como call_sid ou callSid
      const call_sid = body.call_sid || body.callSid || body.CallSid;
      const call_status = body.call_status || body.callStatus || body.CallStatus;
      const sip_status = body.sip_status || body.sipStatus || body.SipStatus;
      const termination_reason = body.termination_reason || body.terminationReason;
      const duration = body.duration || body.Duration || 0;

      logger.info({ call_sid, call_status, sip_status, termination_reason }, 'parsed call status');

      if (!call_sid) {
        logger.info({ body }, 'call_sid is missing from payload');
        return;
      }
