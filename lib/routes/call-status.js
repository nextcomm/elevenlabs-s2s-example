...
module.exports = ({ logger, app }) => {
  app.all('/call-status', async (req, res) => {
    ...
    try {
      ...
      const sip_status = body.sip_status || body.sipStatus;
      const termination_reason = body.termination_reason || body.terminationReason;
      const duration = body.duration || 0;

      logger.info(`PARSED: call_sid=${call_sid} status=${call_status} sip=${sip_status} reason=${termination_reason}`);

      if (!call_sid) {
        logger.info('call_sid missing — check METHOD/BODY/QUERY above');
        return;
      }
